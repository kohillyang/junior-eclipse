...
uint8_t  shangweiji_buf[SHANGWEIJI_BUFLEN];
...
void shangweiji_init()
{
    __HAL_UART_CLEAR_IDLEFLAG(&SWJ_HUART);
    __HAL_UART_ENABLE_IT(&SWJ_HUART, UART_IT_IDLE);
    UART_Receive_DMA_No_IT(&SWJ_HUART, shangweiji_buf, SHANGWEIJI_BUFLEN);
}
...
extern char PID1OK[3];
extern char POSITION_BUF[32];
...
void MyUartFrameIRQHandler(UART_HandleTypeDef *huart)
{
	...
	else if (huart == &CV_HUART)
	{
		...
		else if (huart == &SWJ_HUART)
	{
		SHANGWEIJI_DATAHANDLER();
		testGameInfo.gpsData.x=24.0f;
		testGameInfo.gpsData.y=30.0f;
		testGameInfo.gpsData.compass=270.0f;
		 float sum=0xAA+testGameInfo.gpsData.x+testGameInfo.gpsData.y+testGameInfo.gpsData.compass+0xFF;
		sprintf(POSITION_BUF,"%d %.2f %.2f %.2f %.2f %.2f,%d",0xAA,testGameInfo.gpsData.x,testGameInfo.gpsData.y,testGameInfo.gpsData.compass,sum,0XFF);
